...
$pedido_delete = NULL; // Initialize page object first

class cpedido_delete extends cpedido {
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("pedidolist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in pedido class, pedidoinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("pedidolist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($pedido_delete)) $pedido_delete = new cpedido_delete();

// Page init
$pedido_delete->Page_Init();

// Page main
$pedido_delete->Page_Main();
...
var CurrentForm = fpedidodelete = new ew_Form("fpedidodelete", "delete");

// Form_CustomValidate event
fpedidodelete.Form_CustomValidate = 
...
fpedidodelete.ValidateRequired = true;
<?php } else { ?>
fpedidodelete.ValidateRequired = false; 
<?php } ?>
...
?>
<form name="fpedidodelete" id="fpedidodelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($pedido_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $pedido_delete->Token ?>">
<?php } ?>
...
<?php if ($pedido->id->Visible) { // id ?>
		<th><span id="elh_pedido_id" class="pedido_id"><?php echo $pedido->id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($pedido->empleado->Visible) { // empleado ?>
		<th><span id="elh_pedido_empleado" class="pedido_empleado"><?php echo $pedido->empleado->FldCaption() ?></span></th>
<?php } ?>
<?php if ($pedido->fecha->Visible) { // fecha ?>
		<th><span id="elh_pedido_fecha" class="pedido_fecha"><?php echo $pedido->fecha->FldCaption() ?></span></th>
<?php } ?>
...
while (!$pedido_delete->Recordset->EOF) {
	...
	// Set row properties
	$pedido->ResetAttrs();
	$pedido->RowType = EW_ROWTYPE_VIEW; // View
...
<span id="el<?php echo $pedido_delete->RowCnt ?>_pedido_id" class="pedido_id">
...
<span id="el<?php echo $pedido_delete->RowCnt ?>_pedido_empleado" class="pedido_empleado">
...
<span id="el<?php echo $pedido_delete->RowCnt ?>_pedido_fecha" class="pedido_fecha">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $pedido_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fpedidodelete.Init();
