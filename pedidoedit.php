...
$pedido_edit = NULL; // Initialize page object first

class cpedido_edit extends cpedido {
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("pedidolist.php"); // Invalid key, return to list
		...
		switch ($this->CurrentAction) {
			...
			Case "U": // Update
				...
				if (ew_GetPageName($sReturnUrl) == "pedidolist.php")
...
// Create page object
if (!isset($pedido_edit)) $pedido_edit = new cpedido_edit();

// Page init
$pedido_edit->Page_Init();

// Page main
$pedido_edit->Page_Main();
...
var CurrentForm = fpedidoedit = new ew_Form("fpedidoedit", "edit");
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $pedido->empleado->FldCaption(), $pedido->empleado->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_fecha");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $pedido->fecha->FldCaption(), $pedido->fecha->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_fecha");
			if (elm && !ew_CheckEuroDate(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($pedido->fecha->FldErrMsg()) ?>");
...
fpedidoedit.Form_CustomValidate = 
...
fpedidoedit.ValidateRequired = true;
<?php } else { ?>
fpedidoedit.ValidateRequired = false; 
<?php } ?>
...
?>
<form name="fpedidoedit" id="fpedidoedit" class="<?php echo $pedido_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($pedido_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $pedido_edit->Token ?>">
<?php } ?>
...
<?php if ($pedido->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_pedido_id" class="col-sm-2 control-label ewLabel"><?php echo $pedido->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $pedido->id->CellAttributes() ?>>
<span id="el_pedido_id">
<span<?php echo $pedido->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $pedido->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="pedido" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($pedido->id->CurrentValue) ?>">
<?php echo $pedido->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($pedido->empleado->Visible) { // empleado ?>
	<div id="r_empleado" class="form-group">
		<label id="elh_pedido_empleado" for="x_empleado" class="col-sm-2 control-label ewLabel"><?php echo $pedido->empleado->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $pedido->empleado->CellAttributes() ?>>
<span id="el_pedido_empleado">
<input type="text" data-table="pedido" data-field="x_empleado" name="x_empleado" id="x_empleado" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($pedido->empleado->getPlaceHolder()) ?>" value="<?php echo $pedido->empleado->EditValue ?>"<?php echo $pedido->empleado->EditAttributes() ?>>
</span>
<?php echo $pedido->empleado->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($pedido->fecha->Visible) { // fecha ?>
	<div id="r_fecha" class="form-group">
		<label id="elh_pedido_fecha" for="x_fecha" class="col-sm-2 control-label ewLabel"><?php echo $pedido->fecha->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $pedido->fecha->CellAttributes() ?>>
<span id="el_pedido_fecha">
<input type="text" data-table="pedido" data-field="x_fecha" data-format="7" name="x_fecha" id="x_fecha" placeholder="<?php echo ew_HtmlEncode($pedido->fecha->getPlaceHolder()) ?>" value="<?php echo $pedido->fecha->EditValue ?>"<?php echo $pedido->fecha->EditAttributes() ?>>
</span>
<?php echo $pedido->fecha->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($pedido->cliente->Visible) { // cliente ?>
	<div id="r_cliente" class="form-group">
		<label id="elh_pedido_cliente" for="x_cliente" class="col-sm-2 control-label ewLabel"><?php echo $pedido->cliente->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $pedido_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
