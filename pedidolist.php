...
$pedido_list = NULL; // Initialize page object first

class cpedido_list extends cpedido {
	...
	// Page object name
	var $PageObjName = 'pedido_list';

	// Grid form hidden field names
	var $FormName = 'fpedidolist';
	...
	function __construct() {
		...
		$this->AddUrl = "pedidoadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "pedidodelete.php";
		$this->MultiUpdateUrl = "pedidoupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fpedidolistsrch";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fpedidolistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fpedidolistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fpedidolist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fpedidolistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($pedido_list)) $pedido_list = new cpedido_list();

// Page init
$pedido_list->Page_Init();

// Page main
$pedido_list->Page_Main();
...
var CurrentForm = fpedidolist = new ew_Form("fpedidolist", "list");
fpedidolist.FormKeyCountName = '<?php echo $pedido_list->FormKeyCountName ?>';

// Form_CustomValidate event
fpedidolist.Form_CustomValidate = 
...
fpedidolist.ValidateRequired = true;
<?php } else { ?>
fpedidolist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
// Form object for search

var CurrentSearchForm = fpedidolistsrch = new ew_Form("fpedidolistsrch");
</script>
<script type="text/javascript">
...
<?php
	$bSelectLimit = $pedido_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($pedido_list->TotalRecs <= 0)
			$pedido_list->TotalRecs = $pedido->SelectRecordCount();
	} else {
		if (!$pedido_list->Recordset && ($pedido_list->Recordset = $pedido_list->LoadRecordset()))
			$pedido_list->TotalRecs = $pedido_list->Recordset->RecordCount();
	}
	$pedido_list->StartRec = 1;
	if ($pedido_list->DisplayRecs <= 0 || ($pedido->Export <> "" && $pedido->ExportAll)) // Display all records
		$pedido_list->DisplayRecs = $pedido_list->TotalRecs;
	if (!($pedido->Export <> "" && $pedido->ExportAll))
		$pedido_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$pedido_list->Recordset = $pedido_list->LoadRecordset($pedido_list->StartRec-1, $pedido_list->DisplayRecs);

	// Set no record found message
	if ($pedido->CurrentAction == "" && $pedido_list->TotalRecs == 0) {
		if ($pedido_list->SearchWhere == "0=101")
			$pedido_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$pedido_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$pedido_list->RenderOtherOptions();
?>
<?php if ($pedido->Export == "" && $pedido->CurrentAction == "") { ?>
<form name="fpedidolistsrch" id="fpedidolistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($pedido_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fpedidolistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="pedido">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($pedido_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($pedido_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $pedido_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($pedido_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($pedido_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($pedido_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($pedido_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<form name="fpedidolist" id="fpedidolist" class="form-horizontal ewForm ewListForm ewMultiColumnForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($pedido_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $pedido_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="pedido">
<?php if ($pedido_list->TotalRecs > 0) { ?>
<?php
if ($pedido->ExportAll && $pedido->Export <> "") {
...
if ($pedido_list->Recordset && !$pedido_list->Recordset->EOF) {
	...
	$bSelectLimit = $pedido_list->UseSelectLimit;
	if (!$bSelectLimit && $pedido_list->StartRec > 1)
...
while ($pedido_list->RecCnt < $pedido_list->StopRec) {
	$pedido_list->RecCnt++;
	if (intval($pedido_list->RecCnt) >= intval($pedido_list->StartRec)) {
		$pedido_list->RowCnt++;
...
?>
<?php echo $pedido_list->MultiColumnBeginGrid() ?>
<div class="<?php echo $pedido_list->MultiColumnClass ?>"<?php echo $pedido->RowAttributes() ?>>
	...
	<?php if ($pedido->id->Visible) { // id ?>
		<?php if ($pedido->RowType == EW_ROWTYPE_VIEW) { // View record ?>
		<tr>
			<td class="ewTableHeader"><span class="pedido_id">
...
<span id="el<?php echo $pedido_list->RowCnt ?>_pedido_id">
<span<?php echo $pedido->id->ViewAttributes() ?>>
<?php echo $pedido->id->ListViewValue() ?></span>
</span>
</td>
		</tr>
		<?php } else { // Add/edit record ?>
		<div class="form-group pedido_id">
			<label class="col-sm-2 control-label ewLabel"><?php echo $pedido->id->FldCaption() ?></label>
			<div class="col-sm-10"><div<?php echo $pedido->id->CellAttributes() ?>>
<span id="el<?php echo $pedido_list->RowCnt ?>_pedido_id">
<span<?php echo $pedido->id->ViewAttributes() ?>>
<?php echo $pedido->id->ListViewValue() ?></span>
</span>
	...
	<?php if ($pedido->empleado->Visible) { // empleado ?>
		<?php if ($pedido->RowType == EW_ROWTYPE_VIEW) { // View record ?>
		<tr>
			<td class="ewTableHeader"><span class="pedido_empleado">
...
<?php } ?>
			...
			<td<?php echo $pedido->empleado->CellAttributes() ?>>
<span id="el<?php echo $pedido_list->RowCnt ?>_pedido_empleado">
<span<?php echo $pedido->empleado->ViewAttributes() ?>>
<?php echo $pedido->empleado->ListViewValue() ?></span>
</span>
</td>
		</tr>
		<?php } else { // Add/edit record ?>
		<div class="form-group pedido_empleado">
			<label class="col-sm-2 control-label ewLabel"><?php echo $pedido->empleado->FldCaption() ?></label>
			<div class="col-sm-10"><div<?php echo $pedido->empleado->CellAttributes() ?>>
<span id="el<?php echo $pedido_list->RowCnt ?>_pedido_empleado">
<span<?php echo $pedido->empleado->ViewAttributes() ?>>
<?php echo $pedido->empleado->ListViewValue() ?></span>
</span>
	...
	<?php if ($pedido->fecha->Visible) { // fecha ?>
		<?php if ($pedido->RowType == EW_ROWTYPE_VIEW) { // View record ?>
		<tr>
			<td class="ewTableHeader"><span class="pedido_fecha">
...
<?php } ?>
			...
			<td<?php echo $pedido->fecha->CellAttributes() ?>>
<span id="el<?php echo $pedido_list->RowCnt ?>_pedido_fecha">
<span<?php echo $pedido->fecha->ViewAttributes() ?>>
<?php echo $pedido->fecha->ListViewValue() ?></span>
</span>
</td>
		</tr>
		<?php } else { // Add/edit record ?>
		<div class="form-group pedido_fecha">
			<label class="col-sm-2 control-label ewLabel"><?php echo $pedido->fecha->FldCaption() ?></label>
			<div class="col-sm-10"><div<?php echo $pedido->fecha->CellAttributes() ?>>
<span id="el<?php echo $pedido_list->RowCnt ?>_pedido_fecha">
<span<?php echo $pedido->fecha->ViewAttributes() ?>>
<?php echo $pedido->fecha->ListViewValue() ?></span>
</span>
	...
	<?php if ($pedido->cliente->Visible) { // cliente ?>
		<?php if ($pedido->RowType == EW_ROWTYPE_VIEW) { // View record ?>
		<tr>
			<td class="ewTableHeader"><span class="pedido_cliente">
...
<?php } ?>
			...
			<td<?php echo $pedido->cliente->CellAttributes() ?>>
...
<?php echo $pedido->cliente->ListViewValue() ?></span>
		...
		<?php } else { // Add/edit record ?>
		<div class="form-group pedido_cliente">
			<label class="col-sm-2 control-label ewLabel"><?php echo $pedido->cliente->FldCaption() ?></label>
			<div class="col-sm-10"><div<?php echo $pedido->cliente->CellAttributes() ?>>
<span id="el<?php echo $pedido_list->RowCnt ?>_pedido_cliente">
<span<?php echo $pedido->cliente->ViewAttributes() ?>>
<?php echo $pedido->cliente->ListViewValue() ?></span>
</span>
...
$pedido_list->ListOptions->Render("body", "", $pedido_list->RowCnt);
?>
</div>
<div class="clearfix"></div>
</div>
...
?>
<?php echo $pedido_list->MultiColumnEndGrid() ?>
<div class="clearfix"></div>
<?php } ?>
...
?>
<div>
<?php if ($pedido->CurrentAction <> "gridadd" && $pedido->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($pedido_list->Pager)) $pedido_list->Pager = new cPrevNextPager($pedido_list->StartRec, $pedido_list->DisplayRecs, $pedido_list->TotalRecs) ?>
...
<!--first page button-->
	...
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $pedido_list->PageUrl() ?>start=<?php echo $pedido_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($pedido_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $pedido_list->PageUrl() ?>start=<?php echo $pedido_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $pedido_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($pedido_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $pedido_list->PageUrl() ?>start=<?php echo $pedido_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($pedido_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $pedido_list->PageUrl() ?>start=<?php echo $pedido_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<?php
	foreach ($pedido_list->OtherOptions as &$option)
