...
$detalle_delete = NULL; // Initialize page object first

class cdetalle_delete extends cdetalle {
	...
	// Page object name
	var $PageObjName = 'detalle_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("detallelist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in detalle class, detalleinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("detallelist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($detalle_delete)) $detalle_delete = new cdetalle_delete();

// Page init
$detalle_delete->Page_Init();

// Page main
$detalle_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$detalle_delete->Page_Render();
?>
...
var CurrentForm = fdetalledelete = new ew_Form("fdetalledelete", "delete");

// Form_CustomValidate event
fdetalledelete.Form_CustomValidate = 
...
fdetalledelete.ValidateRequired = true;
<?php } else { ?>
fdetalledelete.ValidateRequired = false; 
<?php } ?>
...
<?php $detalle_delete->ShowPageHeader(); ?>
<?php
$detalle_delete->ShowMessage();
?>
<form name="fdetalledelete" id="fdetalledelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($detalle_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $detalle_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="detalle">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($detalle_delete->RecKeys as $key) { ?>
...
<?php echo $detalle->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
<?php if ($detalle->id->Visible) { // id ?>
		<th><span id="elh_detalle_id" class="detalle_id"><?php echo $detalle->id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($detalle->pedido->Visible) { // pedido ?>
		<th><span id="elh_detalle_pedido" class="detalle_pedido"><?php echo $detalle->pedido->FldCaption() ?></span></th>
<?php } ?>
...
<?php } ?>
	...
	<tbody>
<?php
$detalle_delete->RecCnt = 0;
$i = 0;
while (!$detalle_delete->Recordset->EOF) {
	$detalle_delete->RecCnt++;
	$detalle_delete->RowCnt++;

	// Set row properties
	$detalle->ResetAttrs();
	$detalle->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$detalle_delete->LoadRowValues($detalle_delete->Recordset);

	// Render row
	$detalle_delete->RenderRow();
?>
	<tr<?php echo $detalle->RowAttributes() ?>>
<?php if ($detalle->id->Visible) { // id ?>
		<td<?php echo $detalle->id->CellAttributes() ?>>
<span id="el<?php echo $detalle_delete->RowCnt ?>_detalle_id" class="detalle_id">
...
<span id="el<?php echo $detalle_delete->RowCnt ?>_detalle_pedido" class="detalle_pedido">
...
<?php
	$detalle_delete->Recordset->MoveNext();
}
$detalle_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $detalle_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fdetalledelete.Init();
