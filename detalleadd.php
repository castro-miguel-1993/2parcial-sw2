...
$detalle_add = NULL; // Initialize page object first

class cdetalle_add extends cdetalle {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "detalleview.php")
	...
	function AddRow($rsold = NULL) {
		...
		// pedido
		$this->pedido->SetDbValueDef($rsnew, $this->pedido->CurrentValue, 0, FALSE);
...
// Create page object
if (!isset($detalle_add)) $detalle_add = new cdetalle_add();

// Page init
$detalle_add->Page_Init();

// Page main
$detalle_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$detalle_add->Page_Render();
?>
...
var CurrentForm = fdetalleadd = new ew_Form("fdetalleadd", "add");

// Validate form
fdetalleadd.Validate = function() {
...
fdetalleadd.Form_CustomValidate = 
...
fdetalleadd.ValidateRequired = true;
<?php } else { ?>
fdetalleadd.ValidateRequired = false; 
<?php } ?>
...
<?php $detalle_add->ShowPageHeader(); ?>
<?php
$detalle_add->ShowMessage();
?>
<form name="fdetalleadd" id="fdetalleadd" class="<?php echo $detalle_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($detalle_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $detalle_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $detalle_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
