...
class cempresa_list extends cempresa {
	...
	function __construct() {
		...
		if (!isset($GLOBALS["empresa"]) || get_class($GLOBALS["empresa"]) == "cempresa") {
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fempresalistsrch";
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fempresalist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
...
var CurrentForm = fempresalist = new ew_Form("fempresalist", "list");
...
<?php
	$bSelectLimit = $empresa_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($empresa_list->TotalRecs <= 0)
			$empresa_list->TotalRecs = $empresa->SelectRecordCount();
	} else {
		if (!$empresa_list->Recordset && ($empresa_list->Recordset = $empresa_list->LoadRecordset()))
	...
	if ($empresa->CurrentAction == "" && $empresa_list->TotalRecs == 0) {
		if ($empresa_list->SearchWhere == "0=101")
			$empresa_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$empresa_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
...
<form name="fempresalist" id="fempresalist" class="form-horizontal ewForm ewListForm ewMultiColumnForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
?>
<?php echo $empresa_list->MultiColumnBeginGrid() ?>
<div class="<?php echo $empresa_list->MultiColumnClass ?>"<?php echo $empresa->RowAttributes() ?>>
	<?php if ($empresa->RowType == EW_ROWTYPE_VIEW) { // View record ?>
	<table class="table table-bordered table-striped">
	<?php } else { // Add/edit record ?>
	<div>
	<?php } ?>
	<?php if ($empresa->id->Visible) { // id ?>
		<?php if ($empresa->RowType == EW_ROWTYPE_VIEW) { // View record ?>
		<tr>
			<td class="ewTableHeader"><span class="empresa_id">
<?php if ($empresa->Export <> "" || $empresa->SortUrl($empresa->id) == "") { ?>
				<div class="ewTableHeaderCaption"><?php echo $empresa->id->FldCaption() ?></div>
<?php } else { ?>
				<div class="ewPointer" onclick="ew_Sort(event,'<?php echo $empresa->SortUrl($empresa->id) ?>',1);">
            	<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $empresa->id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($empresa->id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($empresa->id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
				</div>
<?php } ?>
			</span></td>
			<td<?php echo $empresa->id->CellAttributes() ?>>
<span id="el<?php echo $empresa_list->RowCnt ?>_empresa_id">
<span<?php echo $empresa->id->ViewAttributes() ?>>
<?php echo $empresa->id->ListViewValue() ?></span>
</span>
</td>
		</tr>
		<?php } else { // Add/edit record ?>
		<div class="form-group empresa_id">
			<label class="col-sm-2 control-label ewLabel"><?php echo $empresa->id->FldCaption() ?></label>
			<div class="col-sm-10"><div<?php echo $empresa->id->CellAttributes() ?>>
<span id="el<?php echo $empresa_list->RowCnt ?>_empresa_id">
<span<?php echo $empresa->id->ViewAttributes() ?>>
<?php echo $empresa->id->ListViewValue() ?></span>
</span>
	...
	<?php if ($empresa->nombre->Visible) { // nombre ?>
		<?php if ($empresa->RowType == EW_ROWTYPE_VIEW) { // View record ?>
		<tr>
			<td class="ewTableHeader"><span class="empresa_nombre">
<?php if ($empresa->Export <> "" || $empresa->SortUrl($empresa->nombre) == "") { ?>
				<div class="ewTableHeaderCaption"><?php echo $empresa->nombre->FldCaption() ?></div>
<?php } else { ?>
				<div class="ewPointer" onclick="ew_Sort(event,'<?php echo $empresa->SortUrl($empresa->nombre) ?>',1);">
...
<span id="el<?php echo $empresa_list->RowCnt ?>_empresa_nombre">
...
<?php echo $empresa->nombre->ListViewValue() ?></span>
		...
		<?php } else { // Add/edit record ?>
		<div class="form-group empresa_nombre">
			<label class="col-sm-2 control-label ewLabel"><?php echo $empresa->nombre->FldCaption() ?></label>
...
<?php echo $empresa->nombre->ListViewValue() ?></span>
		...
		<?php } ?>
	<?php } ?>
	<?php if ($empresa->descripcion->Visible) { // descripcion ?>
		<?php if ($empresa->RowType == EW_ROWTYPE_VIEW) { // View record ?>
		<tr>
			<td class="ewTableHeader"><span class="empresa_descripcion">
<?php if ($empresa->Export <> "" || $empresa->SortUrl($empresa->descripcion) == "") { ?>
				<div class="ewTableHeaderCaption"><?php echo $empresa->descripcion->FldCaption() ?></div>
<?php } else { ?>
				<div class="ewPointer" onclick="ew_Sort(event,'<?php echo $empresa->SortUrl($empresa->descripcion) ?>',1);">
            	<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $empresa->descripcion->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($empresa->descripcion->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($empresa->descripcion->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
				</div>
<?php } ?>
			</span></td>
			<td<?php echo $empresa->descripcion->CellAttributes() ?>>
<span id="el<?php echo $empresa_list->RowCnt ?>_empresa_descripcion">
<span<?php echo $empresa->descripcion->ViewAttributes() ?>>
<?php echo $empresa->descripcion->ListViewValue() ?></span>
</span>
</td>
		</tr>
		<?php } else { // Add/edit record ?>
		<div class="form-group empresa_descripcion">
			<label class="col-sm-2 control-label ewLabel"><?php echo $empresa->descripcion->FldCaption() ?></label>
			<div class="col-sm-10"><div<?php echo $empresa->descripcion->CellAttributes() ?>>
<span id="el<?php echo $empresa_list->RowCnt ?>_empresa_descripcion">
<span<?php echo $empresa->descripcion->ViewAttributes() ?>>
<?php echo $empresa->descripcion->ListViewValue() ?></span>
</span>
...
$empresa_list->ListOptions->Render("body", "", $empresa_list->RowCnt);
?>
</div>
<div class="clearfix"></div>
</div>
...
?>
<?php echo $empresa_list->MultiColumnEndGrid() ?>
<div class="clearfix"></div>
<?php } ?>
...
<option value="20"<?php if ($empresa_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="ALL"<?php if ($empresa->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
