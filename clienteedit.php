...
class ccliente_edit extends ccliente {
	...
	function EditRow() {
		...
		$conn = &$this->Connection();
		if ($this->ci->CurrentValue <> "") { // Check field with unique index
			$sFilterChk = "(`ci` = " . ew_AdjustSql($this->ci->CurrentValue, $this->DBID) . ")";
			...
			if ($rsChk === FALSE) {
				...
				$sIdxErrMsg = str_replace("%f", $this->ci->FldCaption(), $Language->Phrase("DupIndex"));
				$sIdxErrMsg = str_replace("%v", $this->ci->CurrentValue, $sIdxErrMsg);
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $cliente->nombre->FldCaption(), $cliente->nombre->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_ci");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $cliente->ci->FldCaption(), $cliente->ci->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_ci");
...
<?php if ($cliente->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_cliente_id" class="col-sm-2 control-label ewLabel"><?php echo $cliente->id->FldCaption() ?></label>
...
<?php if ($cliente->nombre->Visible) { // nombre ?>
	<div id="r_nombre" class="form-group">
		<label id="elh_cliente_nombre" for="x_nombre" class="col-sm-2 control-label ewLabel"><?php echo $cliente->nombre->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $cliente->nombre->CellAttributes() ?>>
<span id="el_cliente_nombre">
<input type="text" data-table="cliente" data-field="x_nombre" name="x_nombre" id="x_nombre" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($cliente->nombre->getPlaceHolder()) ?>" value="<?php echo $cliente->nombre->EditValue ?>"<?php echo $cliente->nombre->EditAttributes() ?>>
</span>
<?php echo $cliente->nombre->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($cliente->ci->Visible) { // ci ?>
	<div id="r_ci" class="form-group">
		<label id="elh_cliente_ci" for="x_ci" class="col-sm-2 control-label ewLabel"><?php echo $cliente->ci->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
