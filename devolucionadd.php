...
$devolucion_add = NULL; // Initialize page object first

class cdevolucion_add extends cdevolucion {
	...
	// Page object name
	var $PageObjName = 'devolucion_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "devolucionview.php")
	...
	function AddRow($rsold = NULL) {
		...
		// fecha
		$this->fecha->SetDbValueDef($rsnew, $this->fecha->CurrentValue, 0, FALSE);
...
// Create page object
if (!isset($devolucion_add)) $devolucion_add = new cdevolucion_add();

// Page init
$devolucion_add->Page_Init();

// Page main
$devolucion_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$devolucion_add->Page_Render();
?>
...
var CurrentForm = fdevolucionadd = new ew_Form("fdevolucionadd", "add");

// Validate form
fdevolucionadd.Validate = function() {
...
fdevolucionadd.Form_CustomValidate = 
...
fdevolucionadd.ValidateRequired = true;
<?php } else { ?>
fdevolucionadd.ValidateRequired = false; 
<?php } ?>
...
<?php $devolucion_add->ShowPageHeader(); ?>
<?php
$devolucion_add->ShowMessage();
?>
<form name="fdevolucionadd" id="fdevolucionadd" class="<?php echo $devolucion_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($devolucion_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $devolucion_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $devolucion_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
